...
public class ReservationSeeder : ISeeder
{
    ...
    private async Task SeedUsersBatched()
    {
        ...
        var firstNames = new[] {"Александр", "Елена", "Дмитрий", "Анна", "Михаил", "Ольга", "Сергей", "Наталья", "Владимир", "Татьяна"};
        var lastNames = new[] {"Иванов", "Петров", "Сидоров", "Козлов", "Новиков", "Морозов", "Петухов", "Обухов", "Калинин", "Лебедев"};
        ...
        for (var i = 0; i < USERS_COUNT; i++)
        {
            ...
            users.Add(new User {Id = Guid.NewGuid(), Details = new Details {FIO = $"{firstName} {lastName}", Description = $"Пользователь {firstName} {lastName}", Socials = GenerateRandomSocials(),},});
    ...
    private async Task SeedVenuesAndSeatsBatched()
    {
        ...
        var venuePrefixes = new[] {"MSK", "SPB", "EKB", "NSK", "KZN", "NN", "CHE", "SMR", "UFA", "RND",};
        var venueNames = new[] {"Центральный зал", "Концертный холл", "Конференц-зал", "Театральный зал", "Большой зал", "Малый зал", "Летняя площадка", "Крытый павильон", "Амфитеатр", "Арена",};
    ...
    private async Task SeedEventsBatched()
    {
        ...
        var concertPerformers = new[] {"Би-2", "Сплин", "Мумий Тролль", "Земфира", "Ленинград", "Каста", "Noize MC", "Баста",};
        var conferenceTopics = new[] {"Технологии будущего", "ИИ в бизнесе", "Экология и устойчивое развитие", "Цифровая трансформация",};
        var conferenceSpeakers = new[] {"Иван Петров", "Мария Сидорова", "Алексей Козлов", "Екатерина Новикова",};
        var eventNames = new[] {"Большой концерт", "Весенний фестиваль", "Техническая конференция", "Онлайн-митап",};

        // Получаем ID площадок из базы
        var venueIds = await _dbContext.Venues.Select(v => new {v.Id, v.SeatsLimit,}).ToListAsync();
    ...
    private async Task SeedReservationsBatched()
    {
        ...
        var eventVenueData = await _dbContext.Events.Select(e => new {e.Id, e.VenueId}).ToListAsync();
        var userIds = await _dbContext.Set<User>().Select(u => u.Id).ToListAsync();
        var seatsByVenue = await _dbContext.Seats
            .GroupBy(s => s.VenueId)
            .Select(g => new {VenueId = g.Key, SeatIds = g.Select(s => s.Id.Value).ToList()})
            .ToListAsync();
        ...
        {
            if (!venueSeatsDict.TryGetValue(eventInfo.VenueId, out var seatIds))
                continue;
    ...
    {
        var socialNetworks = new[] {"VK", "Telegram", "Instagram", "Facebook", "Twitter"};
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            socials.Add(new SocialNetwork {Name = network, Link = $"https://{network.ToLower()}.com/user{_random.Next(1000, 9999)}"});
        }
